...
namespace NotBlockbuster.Components {
    public class Menu {
        ...
        static public void StaffRemoveMovie() { // TODO: remove movie from all users
            ...
            movie_to_delete = MainProgram.movie_collec.SearchMovie(movie_title).Title;
            do {Console.Write("Are you sure you would like to remove all copies of '{0}'? (Y)es / (N)o: ", movie_to_delete);
                user_input = Console.ReadLine().ToLower();
            } while (!( user_input.Equals("y") || user_input.Equals("n") ));
            MainProgram.movie_collec.DeleteMovie(movie_to_delete);
            // Check if the user would like to add another movie
            // Perform this loop until the user has chosen a valid option
            ...
            if (user_input.Equals("y")) {
                ...
                foreach (Movie movie in MainProgram.member.BorrowedMovies) {
                    ...
                    if (movie_to_return.CompareTo(movie) == 0) {
                        MainProgram.member.ReturnMovie(movie_to_return); // add movie to user's list
